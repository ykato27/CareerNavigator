...
def sample_member_competence_with_dates():
    ...
    m1_data = [
        ...
        },
        {
            ...
            "力量コード": "c004",
            "正規化レベル": 2,
            "取得日": (base_date + timedelta(days=150)).strftime("%Y/%m/%d"),
        },
    ]
    ...
    m2_data = [
        {
            ...
            "力量コード": "c001",
            "正規化レベル": 4,
            "取得日": (base_date + timedelta(days=10)).strftime("%Y/%m/%d"),
        },
        {
            "メンバーコード": "m002",
            "力量コード": "c004",
            "正規化レベル": 3,
            "取得日": (base_date + timedelta(days=120)).strftime("%Y/%m/%d"),
        },
        {
            "メンバーコード": "m002",
            "力量コード": "c005",
            "正規化レベル": 2,
            "取得日": (base_date + timedelta(days=250)).strftime("%Y/%m/%d"),
        },
    ]
    ...
    m3_data = [
        {
            ...
            "力量コード": "c001",
            "正規化レベル": 3,
            "取得日": (base_date + timedelta(days=20)).strftime("%Y/%m/%d"),
        },
        {
            "メンバーコード": "m003",
            "力量コード": "c003",
            "正規化レベル": 3,
            "取得日": (base_date + timedelta(days=80)).strftime("%Y/%m/%d"),
        },
        {
            "メンバーコード": "m003",
            "力量コード": "c004",
            "正規化レベル": 3,
            "取得日": (base_date + timedelta(days=180)).strftime("%Y/%m/%d"),
        },
    ]
    ...
    m4_data = [
        {
            ...
            "力量コード": "c002",
            "正規化レベル": 4,
            "取得日": (base_date + timedelta(days=30)).strftime("%Y/%m/%d"),
        },
        {
            "メンバーコード": "m004",
            "力量コード": "c003",
            "正規化レベル": 3,
            "取得日": (base_date + timedelta(days=100)).strftime("%Y/%m/%d"),
        },
    ]
    ...
    m5_data = [
        {
            ...
            "力量コード": "c001",
            "正規化レベル": 5,
            "取得日": (base_date + timedelta(days=5)).strftime("%Y/%m/%d"),
        },
        {
            "メンバーコード": "m005",
            "力量コード": "c004",
            "正規化レベル": 4,
            "取得日": (base_date + timedelta(days=140)).strftime("%Y/%m/%d"),
        },
    ]
...
class TestSkillDependencyAnalyzer:
    ...
    def test_initialization_without_date_column(self, sample_competence_master):
        ...
        member_competence = pd.DataFrame(
            {"メンバーコード": ["m001"], "力量コード": ["c001"], "正規化レベル": [3]}
        )

        with pytest.raises(ValueError, match="'取得日'カラムが必要です"):
            SkillDependencyAnalyzer(
                member_competence=member_competence, competence_master=sample_competence_master
            )

    def test_extract_temporal_transitions(
        self, sample_member_competence_with_dates, sample_competence_master
    ):
        """時系列遷移の抽出テスト"""
        analyzer = SkillDependencyAnalyzer(
            member_competence=sample_member_competence_with_dates,
    ...
    def test_calculate_transition_confidence(
        self, sample_member_competence_with_dates, sample_competence_master
    ):
        """遷移信頼度の計算テスト"""
        analyzer = SkillDependencyAnalyzer(
    ...
    def test_infer_dependency_direction(
        self, sample_member_competence_with_dates, sample_competence_master
    ):
        """依存関係の方向推定テスト"""
        analyzer = SkillDependencyAnalyzer(
    ...
    def test_generate_learning_paths_for_specific_skills(
        self, sample_member_competence_with_dates, sample_competence_master
    ):
        """特定スキルの学習パス生成テスト"""
        analyzer = SkillDependencyAnalyzer(
    ...
    def test_get_dependency_graph_data(
        self, sample_member_competence_with_dates, sample_competence_master
    ):
        """依存関係グラフデータ取得のテスト"""
        analyzer = SkillDependencyAnalyzer(
