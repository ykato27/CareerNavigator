...
class TestInitialization:
    ...
    def test_custom_initialization(self):
        """カスタムパラメータで初期化"""
        reranker = DiversityReranker(lambda_relevance=0.5, category_weight=0.6, type_weight=0.4)
...
class TestMMRReranking:
    ...
    def test_mmr_promotes_diversity(self, sample_candidates, sample_competence_info):
        ...
        types = [
            sample_competence_info[sample_competence_info["力量コード"] == code]["力量タイプ"].iloc[
                0
            ]
    ...
    def test_mmr_k_larger_than_candidates(
        self, reranker, sample_candidates, sample_competence_info
    ):
        """k が候補数より大きい場合"""
...
class TestCategoryDiversity:
    ...
    def test_category_diversity_basic(self, reranker, sample_candidates, sample_competence_info):
        """基本的なカテゴリ多様性"""
        result = reranker.rerank_category_diversity(sample_candidates, sample_competence_info, k=5)

        assert len(result) == 5

    def test_category_diversity_with_max_per_category(
        self, reranker, sample_candidates, sample_competence_info
    ):
        """カテゴリごとの最大数制限"""
        result = reranker.rerank_category_diversity(
            sample_candidates, sample_competence_info, k=8, max_per_category=2
        )

        # カテゴリごとのカウント
        category_counts = {}
        for code, _ in result:
            category = sample_competence_info[sample_competence_info["力量コード"] == code][
                "力量カテゴリー名"
            ].iloc[0]
...
class TestTypeDiversity:
    ...
    def test_type_diversity_basic(self, reranker, sample_candidates, sample_competence_info):
        """基本的なタイプ多様性"""
        result = reranker.rerank_type_diversity(sample_candidates, sample_competence_info, k=6)
    ...
    def test_type_diversity_with_ratios(self, reranker, sample_candidates, sample_competence_info):
        ...
        for code, _ in result:
            comp_type = sample_competence_info[sample_competence_info["力量コード"] == code][
                "力量タイプ"
            ].iloc[0]
...
class TestHybridReranking:
    ...
    def test_hybrid_basic(self, reranker, sample_candidates, sample_competence_info):
        """基本的なハイブリッド再ランキング"""
        result = reranker.rerank_hybrid(sample_candidates, sample_competence_info, k=5)
    ...
    def test_hybrid_promotes_diversity(self, reranker, sample_candidates, sample_competence_info):
        ...
        result = reranker.rerank_hybrid(
            sample_candidates, sample_competence_info, k=6, max_per_category=2
        )
...
class TestDiversityMetrics:
    ...
    def test_calculate_diversity_metrics_basic(
        self, reranker, sample_candidates, sample_competence_info
    ):
        """基本的な多様性指標計算"""
        recommendations = sample_candidates[:5]

        metrics = reranker.calculate_diversity_metrics(recommendations, sample_competence_info)
    ...
    def test_intra_list_diversity_calculation(self, reranker, sample_competence_info):
        ...
        same_metrics = reranker.calculate_diversity_metrics(
            same_recommendations, sample_competence_info
        )

        diverse_metrics = reranker.calculate_diversity_metrics(
            diverse_recommendations, sample_competence_info
        )
...
class TestIntegration:
    ...
    def test_full_diversity_workflow(self, sample_candidates, sample_competence_info):
        ...
        # 4. 多様性指標を計算
        hybrid_metrics = reranker.calculate_diversity_metrics(hybrid_result, sample_competence_info)
