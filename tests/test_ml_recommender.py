...
def sample_competence_master():
    ...
    return pd.DataFrame({
        ...
        '力量カテゴリー名': ['プログラミング', 'プログラミング', 'データベース', 'クラウド', 'インフラ']
    })
...
@pytest.fixture
def ml_recommender(sample_skill_matrix, sample_competence_master, sample_member_competence):
    """MLベース推薦エンジン"""
...
class TestIntegration:
    ...
    def test_full_ml_recommendation_workflow(self, ml_recommender):
        """完全なML推薦ワークフロー"""
        # 1. 基本推薦
        basic_recs = ml_recommender.recommend(
            member_code='m001',
            top_n=5,
            use_diversity=False
        )
