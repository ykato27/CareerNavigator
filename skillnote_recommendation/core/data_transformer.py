...
class DataTransformer:
    ...
    def create_member_competence(self, data: Dict[str, pd.DataFrame],
        ...
        if member_competence.empty:
            ...
            empty_df = pd.DataFrame(columns=[
                member_code_col_acquired, competence_code_col, competence_type_col, level_col,
                '正規化レベル', '力量名', '力量カテゴリー名'
            ])
        ...
        member_competence['正規化レベル'] = member_competence.apply(
            lambda row: self.normalize_level(row[level_col], row[competence_type_col]),
            axis=1
        )
    ...
    def clean_members_data(self, data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        # 必要なカラムキーワードを定義
        column_keywords = ['メンバーコード', 'メンバー名', 'よみがな', '生年月日', '性別',
                          '入社年月日', '社員区分', '役職', '職能・等級', '職種']
