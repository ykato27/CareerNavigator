...
class DataQualityMonitor:
    ...
    def check_consistency(
        ...
        if 'レベル' in member_competence.columns or '正規化レベル' in member_competence.columns:
            ...
            # 範囲外のレベル
            invalid_levels = member_competence[
                ((levels < 0) | (levels > 5)) & levels.notna()
            ]
        ...
        if '取得日' in df.columns:
            ...
            if len(multi_acquisition) > 0:
                issues.append(DataQualityIssue(
                    ...
                    details={
                        'examples': multi_acquisition.head(5).to_dict()
                    },
                    recommendations=[
    ...
    def check_timeliness(self, df: pd.DataFrame) -> List[DataQualityIssue]:
        ...
        if invalid_dates_count > 0:
            issues.append(DataQualityIssue(
                ...
                recommendations=["取得日の形式を確認してください（YYYY/MM/DD または YYYY-MM-DD）"]
            ))
        ...
        if len(valid_dates) > 0:
            ...
            if len(future_dates) > 0:
                issues.append(DataQualityIssue(
                    ...
                    details={
                        'latest_future_date': str(future_dates.max().date())
                    },
    ...
    def _check_prerequisite_violations(
        self,
        member_competence: pd.DataFrame
    ) -> List[Dict]:
        """前提スキルの依存関係違反をチェック"""
        ...
        for member in member_competence['メンバーコード'].unique():
            member_data = member_competence[
                member_competence['メンバーコード'] == member
            ]
            ...
            for advanced_skill, prerequisites in self.skill_dependencies.items():
                if advanced_skill in acquired_skills:
                    missing_prereqs = [
                        ...
                        if prereq not in acquired_skills
                    ]
    ...
    def _generate_report(
        ...
        summary = {
            'data_shape': df.shape,
            'total_members': df['メンバーコード'].nunique() if 'メンバーコード' in df.columns else 0,
