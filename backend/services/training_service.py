...
class TrainingService:
    ...
    async def train_model(
        ...
        members_clean = transformed_data.get("members_clean")
        ...
        try:
            ...
            recommender.session_id = session_id
            if members_clean is not None and not members_clean.empty:
                recommender.member_lookup_ = {
                    str(row.get("メンバーコード", "")): str(row.get("メンバー名", ""))
                    for _, row in members_clean.iterrows()
                    if str(row.get("メンバーコード", "")) and str(row.get("メンバー名", ""))
                }
