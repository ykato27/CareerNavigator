...
async def logging_middleware(request: Request, call_next: Callable) -> Response:
    ...
    logger.info(
        "Request started", query_params=dict(request.query_params) if request.query_params else {}
    )
    ...
    try:
        ...
        logger.info(
            "Request completed", status_code=response.status_code, duration_ms=round(duration_ms, 2)
        )
        ...
        logger.error(
            "Request failed", error_type=type(exc).__name__, duration_ms=round(duration_ms, 2)
        )
        raise
