...
- **例**: 
  - 「2D CAD」の前提スキルが「図面の読み取り」「製図基礎」の2つ
  - あなたが「図面の読み取り」のみ習得済み → Readiness = 0.5 (50%)
...
- **例**:
  - 組織の90%の製造技術者が「2D CAD」を習得 → Probability = 0.9
...
- **例**:
  - 目標が「設計リーダー」で、そのロールが「3D CAD」を必須保有 → Utility = 1.0
...
|--------|-----------|-------------|---------|-----------|------|
| 2D CAD | 1.0 | 0.9 | 0.95 | **0.95** | ✅ 前提揃い、一般的、有用 → 最優先推薦 |
| PLC制御 | 0.4 | 0.6 | 1.0 | 0.60 | ⚠️ 前提不足（電気回路基礎が必要） → まだ早い |
| ISO9001品質管理 | 0.7 | 0.8 | 0.6 | 0.70 | 💡 学びやすく標準的だが目標への関連度は中程度 |
