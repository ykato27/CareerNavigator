...
⚠️ 注意: この機能は Inference ページの「モデル比較モード」に統合されました。
今後は Inference ページをご利用ください。
"""
...
def main():
    ...
    このモデル比較機能は **Inference ページ** に統合されました。

    Inferenceページでは、「🔬 モデル比較モード」を有効にすることで、
    より充実した比較機能（NMF、グラフベース、ハイブリッド推薦の3つを比較可能）をご利用いただけます。

    👉 今後は **Inference ページ** のご利用をお勧めします。
    """)
