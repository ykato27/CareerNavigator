...
export const DataUpload = () => {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            sessionStorage.removeItem('career_model_id');
            sessionStorage.removeItem('career_model_trained');
            if (response.data.expires_at) {
                sessionStorage.setItem('career_session_expires_at', response.data.expires_at);
        ...
        <div className="space-y-6">
            ...
            {success && (
                <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg flex items-center gap-2">
                    <CheckCircle size={20} />
                    <div>
                        <div>データの読み込みが完了しました。メニューから分析機能を利用できます。</div>
                        {expiresAt && (
                            <div className="text-sm mt-1">
                                セッション保持期限: {new Date(expiresAt).toLocaleString('ja-JP')}
                            </div>
                        )}
                    </div>
                </div>
