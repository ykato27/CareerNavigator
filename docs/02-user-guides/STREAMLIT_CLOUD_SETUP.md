...
Streamlit Cloudは以下のファイルを自動的に検出し、依存関係をインストールします：

- `requirements.txt` ← **優先される**
- `pyproject.toml` (requirements.txtがない場合)
- `packages.txt` (システムパッケージ用)

**このプロジェクトでは `requirements.txt` を使用しています。**

必要なパッケージ（Optunaを含む）は既に記載されています：
```txt
pandas>=2.0.0
numpy>=1.24.0
scikit-learn>=1.3.0
