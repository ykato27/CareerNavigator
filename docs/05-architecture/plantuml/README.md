...
- プレゼンテーション層（WebUI: React + Pages Functions）
- アプリケーション層（ビジネスロジック）
...
- API レイヤー（Pages Functions）の主要エンドポイント
...
- API モデル（TrainParams, RecommendRequest等）
...
**概要**:
- 因果推論推薦システムの実行フロー
  1. データアップロードフェーズ（CSV → Pages Functions）
...
- 因果推論パイプラインの動的な振る舞い
- React ↔ Pages Functions 間のデータフロー
- LiNGAMアルゴリズムの実行プロセス
...
- API レイヤー（Pages Functions, D1, R2）
...
- [WebUI ガイド](../../03-webui/WEBUI_GUIDE.md) - 現行 WebUI の構成
- [テスト実行ガイド](../TESTING_QUICKSTART.md) - テストの実行方法

**運用・デプロイ**:
- [プロジェクト README](../../../README.md) - ローカル起動と Cloudflare デプロイ手順
