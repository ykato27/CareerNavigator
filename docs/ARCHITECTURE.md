...
│  ┌──────────────────┐    ┌──────────────────────┐       │
│  │ Streamlit UI     │    │  WebUI               │       │
│  │  - データ読み込み   │    │   Frontend (React)   │       │
│  │  - 因果推論推薦     │    │   Backend (FastAPI)  │       │
│  │  - キャリアダッシュボード│  │                      │       │
│  │  - 組織スキルマップ  │    │                      │       │
│  └──────────────────┘    └──────────────────────┘       │
└──────────────────────────────────────────────────────────┘
...
- `ml/hierarchical_bayesian_recommender.py`: 階層的ベイジアン推薦
...
#### 因果推論ベース推薦
- `graph/causal_graph_recommender.py`: 因果グラフ推薦
- `graph/causal_career_path.py`: 因果キャリアパス
- `graph/dependency_analyzer.py`: 依存関係分析
