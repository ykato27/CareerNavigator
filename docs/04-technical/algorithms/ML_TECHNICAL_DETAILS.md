...
def load_ml_model(data_key, data):
    """MLモデルをメモリキャッシュ"""
    if data_key not in _model_cache:
        _model_cache[data_key] = MLRecommender(data)
    return _model_cache[data_key]

# 使用例
ml_recommender = load_ml_model("session-001", raw_data)
```
