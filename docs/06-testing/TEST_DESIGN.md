...
**バージョン**: 3.0  
**最終更新**: 2025-12-04  
**対象プロジェクト**: CareerNavigator WebUI（React + Pages Functions）
...
このドキュメントは、CareerNavigator WebUI の包括的なテスト設計を定義します。品質保証とリグレッション防止のため、現行のフロントエンドと API 層、ならびに残存する Python 参照実装のテスト方針を整理します。
...
- API 層: Cloudflare Pages Functions
- コアライブラリ: skillnote_recommendation
- 参照実装: backend (FastAPI)
...
| **統合テスト** | APIエンドポイントの動作検証 | Pages Functions / legacy backend routes | 高 |
